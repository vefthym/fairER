...
def load_data(lang,train_ratio,fold):             
    entity1,rel1,triples1 = load_triples(lang + 'rel_triples_1')
    entity2,rel2,triples2 = load_triples(lang + 'rel_triples_2')

    train_pair = []
    with open(lang + "721_5fold/" + str(fold) + "/train_links") as fp1:
        for line in fp1:
            train_pair.append((int(line.split("\t")[0]), int(line.split("\t")[1].rstrip())))

    dev_pair = []
    with open(lang + "721_5fold/" + str(fold) + "/test_links") as fp2:
        for line in fp2:
            dev_pair.append((int(line.split("\t")[0]), int(line.split("\t")[1].rstrip())))

    valid_pair = []
    with open(lang + "721_5fold/" + str(fold) + "/valid_links") as fp3:
        for line in fp3:
            valid_pair.append((int(line.split("\t")[0]), int(line.split("\t")[1].rstrip())))

    adj_matrix,r_index,r_val,adj_features,rel_features = get_matrix(triples1+triples2,entity1.union(entity2),rel1.union(rel2))
    return entity1, entity2, np.array(train_pair),np.array(dev_pair), np.array(valid_pair),adj_matrix,np.array(r_index),np.array(r_val),adj_features,rel_features
