...
from matching.bert_int import bert_int_pre as bpre
...
def runFairER_RREA(dataset, explanation, conf, k=20, method="RREA"):

    cur_dir = os.path.abspath(".")
    which_entity = 0
    main_RREA_fairER.main(dataset, conf, which_entity, k, method)
...
def runUnfair_RREA(dataset, explanation, conf, k=20, method="RREA"):
    cur_dir = os.path.abspath(".")
    which_entity = 0
    main_RREA_unfair.main(dataset, conf, which_entity, k, method)
...
def runStatistics(dataset, dataset_type, conf):
    """
    ...
    if dataset_type == "tab":
        ...
        statistics_1.main(dataset, conf)
...
def saveNewCond(dataset, condition, condition_w_exp):
    ...
    pickle_w_exp_path = os.path.join(os.getcwd(), 'web','data', 'pickle_data', 'protected_conditions_w_exp.pkl')
    # curr_dir = os.path.split(os.getcwd())[1]
    # if curr_dir == 'fairER':
        # pickle_path = 'web/' + pickle_path
...
def download_kg_dataset():
    ...
    url = 'https://drive.google.com/u/0/uc?id=118e-N8RzDL8p4mdYTCUpFC0jVuiSlcec&export=download'
...
def convert_into_ids(dataset, src_path, dest_path):
    convert_to_ids.run(dataset, src_path, dest_path)

def convert_to_bert_int(dataset, src_path, dest_path):
    bpre.preprocess_for_bert_int(src_path, dest_path)

def read_uploaded_dataset(dataset, dataset_type):
    if dataset_type == "kg":
        read_kg_datasets()
        cur_dir = os.path.abspath(".")
        src_path = os.path.join(cur_dir,  'resources', 'Datasets', dataset)
        dest_path = os.path.join(cur_dir,  'resources', 'Datasets', dataset + "_RREA")
        convert_into_ids(dataset, src_path, dest_path)
        src_path_bert = os.path.join(cur_dir,  'resources', 'Datasets', dataset)
        dest_path_bert = os.path.join(cur_dir,  'resources', 'Datasets', dataset + "_BERT_INT")
        convert_to_bert_int(dataset, src_path_bert, dest_path_bert)
    else:
...
def construct_cond_kg(min_size_thres, operator):

    return "(comp_size(ent1) < " + min_size_thres + ") " + operator + " (comp_size(ent2) < " + min_size_thres + ")"
