...
function expl_pop(){
    ...
    if(dataset_type == "kg"){
        $("#explanation-check-container").hide();
    }
}
...
function has_cached_data() {
    ...
    if(dataset_type == "tab") {
        ...
        $('#pair-is-prot').addClass("visible")
        $('#protected-tuples-tab').html("Protected Tuples")
    }
    ...
    if(dataset_type == "kg") {
        ...
        $('#pair-is-prot').addClass("hidden")
        $('#protected-tuples-tab').html("Fairness Conditions")
    }
...
function upload_dataset_info() {
    var htmlRes = '<div id="upload-dataset">'
        ...
        + '<button type="button" class="btn btn-success" onclick="upload_dataset(\'' + "tab" + '\')">Upload</button>'
        + '</form>'
        + '</div>'
        + '</div>'
...
function upload_kg_info(){
    var htmlRes = '<div id="upload-kg">'
    + '<div id="upload-instructions-kg">'
    + '<p><b>Instructions to upload your KG dataset</b></p>'
    + '<ol>'
    + '<li>The dataset should be a zip file.</li>'
    + '<li>The zip file name will be the name of the dataset.</li>'
    + '<li>The zip file should contain the following files'
    + '<ul>'
    + '<li>rel_triples_1</li>'
    + '<li>rel_triples_2</li>'
    + '<li>attr_triples_1</li>'
    + '<li>attr_triples_2</li>'
    + '<li>721_5fold/2/ent_links</li>'
    + '<li>721_5fold/2/train_links</li>'
    + '<li>721_5fold/2/valid_links</li>'
    + '<li>721_5fold/2/test_links</li>'
    // + '<li>ent_ids_1 (in case of RREA)</li>'
    // + '<li>ent_ids_2 (in case of RREA)</li>'
    // + '<li>rel_ids_1 (in case of RREA)</li>'
    // + '<li>rel_ids_2 (in case of RREA)</li>'
    + '</ul>'
    + '</li>'
    + '<li>Your dataset should be structured as '
    + '<a href="https://github.com/nju-websoft/OpenEA#dataset-overview"'
    + 'target="_blank">OpenEA Datasets</a>'
    // + ' or <a href="https://github.com/MaoXinn/RREA/tree/master/CIKM" target="_blank">RREA Datasets</a>.</li>'
    + '<li>Once you upload your dataset, it will be available on the dataset selection dropdown.</li>'
    + '</ol>'
    + '</div>'
    + '<div class="mb-3">'
    + '<form enctype="multipart/form-data" id="kg-upload-form">'
    + '<label for="kg-upload-file" class="form-label">Upload your dataset</label>'
    + '<input class="form-control" name="kg-upload-file" type="file" id="kg-upload-file" accept=".zip">'
    + '<button type="button" class="btn btn-success" onclick="upload_dataset(\'' + "kg" + '\')">Upload</button>'
    + '</form>'
    + '</div>'
    + '</div>'
$('#datasets-info-container').append(htmlRes)
}
...
function edit_condition() {
    ...
    var dataset = $('#dataset-val').val()
    if(dataset.startsWith("D_W") || dataset.startsWith("D_Y")){
        get_condition('current-protected-condition')
        show_condition_input_fields_kg('protected-container');
    }else{
        get_condition('current-protected-condition')
...
function show_condition_input_fields_kg(container_id){

    get_condition(null, function (condition, error) {
        if (error) {
            console.error(error);
        } else {
            var min_size_thres = condition.match(/\d+/g)[1];
            ...
            left_label.innerHTML = '<b>Component Size Threshold</b>'
            left_label.className = "form-label";
            // left_label.htmlFor = "comp_size";

            let sl = document.createElement('div');
            sl.id = 'number-slider';

            let sl_value = document.createElement('p')
            sl_value.id = "slider-p"
            sl_value.textContent  = min_size_thres

            left_tuple_container.appendChild(left_label);
            left_tuple_container.appendChild(sl);
            left_tuple_container.appendChild(sl_value);

            let operator_container = document.createElement('div');
            operator_container.className = 'op-cont';
            ...
            log_op_option1.text = operator1;
            log_op_option1.value = operator1;
            logical_op.add(log_op_option1);
            let log_op_option2 = document.createElement('option');
            log_op_option2.text = operator2;
            log_op_option2.value = operator2;
            logical_op.add(log_op_option2);

            let operator_label = document.createElement('label');
            operator_label.innerHTML = '<b>Operator</b><br>'
            operator_label.className = "form-label";
            operator_container.appendChild(operator_label)
            operator_container.appendChild(logical_op)

            container.appendChild(left_tuple_container);
            container.appendChild(operator_container);
            ...
            $("#number-slider").html('<input id="slider-input" type="range" min="1" max="100" value="' + min_size_thres + '">');
            $("#slider-input").on('input', function(){
                sl_value.textContent = $("#slider-input").val();
            })
        }
    });
...
function expl_description(container_id){

    descr = 'Suggested Matches try to equally represent Protected and Non-Protected Candidates in the top-k positions.';

    let container = document.createElement("div"); 
    container.className = "expl-description";
    
    let title = document.createElement("h1");
    title.innerHTML = 'Explanations Description';
    
    let accuracy_container =  document.createElement("div");
    accuracy_container.className = 'descr-container';
    ...
    accuracy_descr.innerHTML = descr;
    accuracy_container.appendChild(accuracy_descr);

    container.appendChild(title);
    container.appendChild(accuracy_container);
    document.getElementById(container_id).appendChild(container);
}

function show_data_descr(){
    if($(".data-info-btn").text() == "Info"){
        statistics_description();
        $(".data-info-btn").text('Hide')
        $(".data-info-btn").addClass("btn-hide")
    }else{
        $(".stats-description").hide()
        $(".data-info-btn").text('Info')
        $(".data-info-btn").removeClass("btn-hide")
    }
}

function statistics_description(){

    descr = '<ul><li><b>#Ents</b> is the number of entities in the two KGs</li><li><b>#Rels</b> is the number of relations types in the two KGs</li><li><b>#Triples</b> is the number of relation triples in the two KGs</li><li><b>wccR</b> is the ratio of weakly connected components in the two KGs</li><li><b>maxCS</b> is the maximum component size in the two KGs</li><li><b>deg</b> is the average node degree in the two KGs</li></ul>'
    
    let container = document.createElement("div"); 
    container.className = "stats-description";

    let title = document.createElement("h1");
    title.innerHTML = 'Statistics Description';
    
    let accuracy_container =  document.createElement("div");
    accuracy_container.className = 'stat-descr-container';
    let accuracy_title = document.createElement("h2");
    accuracy_container.appendChild(accuracy_title);
    let accuracy_descr = document.createElement("p");
    accuracy_descr.innerHTML = descr;
    ...
    container.appendChild(title);
    container.appendChild(accuracy_container);
    document.getElementById("datasets-info-container").appendChild(container);
}
